...
//Andriod platform doesn't support uid&gid, so the function getpwnam, getpwuid, getgrnam, getgrgid are not useful.
//Need to refactor the function sys_getpwnam, getpwuid, getgrnam, getgrgid.
#ifdef ADD_ANDROID_SUPPORT
#define AID_ROOT 0
...
static char *systemmem[] = {
	"system"
};
static char *sdcardmem[] = {
	"sdcard_rw"
};
static char *inetmem[] = {
	"inet"
...
static struct group systemgr = {
	"system",
	"system",
	AID_SYSTEM,
	systemmem
};
static struct group sdcardgr = {
	"sdcard_rw",
	"sdcard_rw",
	AID_SDCARD_RW,
	sdcardmem
};
static struct group inetgr = {
	"inet",
	"inet",
	AID_INET,
	inetmem
...
struct passwd *sys_getpwnam(const char *name)
{
#ifndef ADD_ANDROID_SUPPORT
	return getpwnam(name);
#else
	if (!strcmp(name,"system")) {
		return &systempw;
	}
	else if (!strcmp(name,"sdcard_rw")) {
		return &sdcardpw;
	}
	else if (!strcmp(name,"inet")) {
		return &inetpw;
	}
	else {
		return &rootpw;
...
struct passwd *sys_getpwuid(uid_t uid)
{
	...
	else if (uid == AID_SDCARD_RW) {
		return &sdcardpw;
	}
	else if (uid == AID_INET) {
		return &inetpw;
	}
	else {
		return &rootpw;
...
struct group *sys_getgrgid(gid_t gid)
{
	...
	else if (gid == AID_SDCARD_RW) {
		return &sdcardgr;
	}
	else if (gid == AID_INET) {
		return &inetgr;
	}
	else {
		return &rootgr;
