...
#ifdef CHECK_OLD_LOGIN
//The 32 hex hash code is from (77-32) to 77, so get 77 bytes
#define SMBPASSWD_FILE_SIZE	77
//The old password file path
...
#ifdef CHECK_OLD_LOGIN
static BOOL get_old_password(const uchar *part_passwd_old)
{
	char buffer[SMBPASSWD_FILE_SIZE] = {0};
	char *p;
	const char *pfile = SMBPASSWD_FILE_OLD;
	int fd;
	int ret;

	fd = open(pfile, O_RDONLY);
	if (fd < 0) {
		DEBUG(5,("Samba_server:old_password:There is no old password file\n"));
		return False;
	}
	ret = read(fd, buffer, SMBPASSWD_FILE_SIZE);
	close(fd);
	if(ret != SMBPASSWD_FILE_SIZE) {
		DEBUG(5,("Samba_server:old_password:Failed to get_old_password\n"));
		return False;
	}
	p = buffer + SMBPASSWD_FILE_SIZE -32;
	pdb_gethexpwd(p, part_passwd_old);
	return True;
}
#endif
				 ...
				 DATA_BLOB *user_sess_key)
{
	...
	BOOL res_old;
	uchar kr_old[16];
	uchar value_from_encryption_old[16];
	uchar part_passwd_old[16];
#endif
	if (part_passwd == NULL) {
		DEBUG(10,("No password set - DISALLOWING access\n"));
		/* No password set - always False */
...
#ifdef CHECK_OLD_LOGIN
	res_old = get_old_password(part_passwd_old);
#endif
	if (sec_blob->length != 8) {
...
#ifdef CHECK_OLD_LOGIN
	if(res_old) {
		if (!ntv2_owf_gen(part_passwd_old, user, domain, upper_case_domain, kr_old)) {
			DEBUG(5,("Samba_server:old_password:Failed to generate kr_old\n"));
		}
		SMBOWFencrypt_ntv2(kr_old, sec_blob, &client_key_data, value_from_encryption_old);
	}
#endif
...
#if DEBUG_PASSWORD
	DEBUG(5,("Part password (P16) was |\n"));
	dump_data(5, (const char *)part_passwd, 16);
	DEBUGADD(5,("Password from client was |\n"));
	dump_data(5, (const char *)ntv2_response->data, ntv2_response->length);
	DEBUGADD(5,("Variable data from client was |\n"));
	dump_data(5, (const char *)client_key_data.data, client_key_data.length);
	DEBUGADD(5,("Given challenge was |\n"));
	dump_data(5, (const char *)sec_blob->data, sec_blob->length);
	DEBUGADD(5,("Value from encryption was |\n"));
	dump_data(5, (const char *)value_from_encryption, 16);
#ifdef CHECK_OLD_LOGIN
	if(res_old) {
		DEBUGADD(5,("Samba_server:old_password:Part password(old)(P16) was |\n"));
		dump_data(5, (const char *)part_passwd_old, 16);
		DEBUGADD(5,("Samba_server:old_password:Value from encryption(old) was |\n"));
		dump_data(5, (const char *)value_from_encryption_old, 16);
	}
#endif
#endif
...
#ifdef CHECK_OLD_LOGIN
	if(!res && res_old) {
		if(memcmp(value_from_encryption_old, client_response, 16) == 0) {
			DEBUG(5,("Samba_server:old_password:This password is old one\n"));
			send_unix_socket(OLD_PASSWOED_LOGIN, smbd_child_fd(), NULL);
		} else {
			DEBUG(5,("Samba_server:old_password:This password is not old one\n"));
		}
	}
#endif
