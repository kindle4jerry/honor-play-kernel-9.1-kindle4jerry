...
NTSTATUS smb_register_charset_smbpw(struct charset_functions *funcs) 
{
	if (!funcs) {
...
static void lazy_initialize_iconv(void)
{
	...
	if (!initialized) {
		...
		for(i = 0; builtin_functions[i].name; i++) 
			smb_register_charset_smbpw(&builtin_functions[i]);
		static_init_charset;
	}
}
 ...
 **/
size_t smb_iconv_smbpw(smb_iconv_t cd, 
		 const char **inbuf, size_t *inbytesleft,
		 char **outbuf, size_t *outbytesleft)
{
	char cvtbuf[2048];
 ...
 */
smb_iconv_t smb_iconv_open_smbpw(const char *tocode, const char *fromcode)
{
	smb_iconv_t ret;
...
*/
int smb_iconv_close_smbpw (smb_iconv_t cd)
{
#ifdef HAVE_NATIVE_ICONV
