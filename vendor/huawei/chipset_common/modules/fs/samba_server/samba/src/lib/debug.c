...
**************************************************************************/
#ifdef ADD_LOG_SIZE_CONTROL
static int check_log_files(void){
	SMB_STRUCT_DIR *dir;
	struct stat statbuf;
	char *logfname = NULL;
	char *directory = NULL;
	...
	max_log_files = lp_max_log_files();
	if (0 == max_log_files) {
		return 0;
	}
	logfname = lp_logfile();
	if(NULL != logfname) {
		directory = (char *)SMB_MALLOC(strlen(logfname) +1);
		strncpy(directory, logfname, strlen(logfname));
		directory[strlen(logfname)] = '\0';
	}
	dirname(directory);
	dir = sys_opendir(directory);
	if (!dir) {
		DEBUG(0, ("Unable to open directory: [%s].\n", directory));
		SAFE_FREE(directory);
		return -1;
	}
	pathlist = (pathinfo_t**)SMB_MALLOC(sizeof(pathinfo_t*) * (max_log_files + 1));
	while ((dname = readdirname(dir))) {
		if (!strcmp(dname,".") || !strcmp(dname,".."))
			continue;
		if (log_index < max_log_files) {
			pathlist[log_index] = (pathinfo_t*)SMB_MALLOC(sizeof(pathinfo_t));
			pathlist[log_index]->pathname = NULL;
			if (asprintf(&pathlist[log_index]->pathname, "%s/%s", directory, dname) <= 0) {
				continue;
			}
			if (stat(pathlist[log_index]->pathname, &statbuf) < 0) {
				DEBUG(0, ("Unable to get stat: [%s]\n", pathlist[log_index]->pathname));
				ret = -1;
			}
			pathlist[log_index]->mtime = statbuf.st_mtime;
			for (i = 0; i < log_index; i++) {
				if (pathlist[log_index]->mtime > pathlist[i]->mtime) {
					path_tmp = pathlist[log_index];
					for (j = log_index; j > i; j--) {
		...
		if (log_index >= max_log_files) {
			if (asprintf(&pathlist[max_log_files]->pathname, "%s/%s", directory, dname) <= 0) {
				continue;
			}
			if (stat(pathlist[max_log_files]->pathname, &statbuf) < 0) {
				DEBUG(0, ("Unable to get stat: [%s]\n", pathlist[max_log_files]->pathname));
				ret = -1;
			}
			pathlist[max_log_files]->mtime = statbuf.st_mtime;
			for (i = 0; i < max_log_files; i++) {
				if (pathlist[max_log_files]->mtime > pathlist[i]->mtime) {
					path_tmp = pathlist[max_log_files];
					for (j = max_log_files; j > i; j--) {
						pathlist[j] = pathlist[j-1];
					}
					pathlist[i] = path_tmp;
					unlink(pathlist[max_log_files]->pathname);
					SAFE_FREE(pathlist[max_log_files]->pathname);
					break;
				}
	...
	if (log_index < max_log_files) {
		for (i = 0; i <= log_index; i++) {
			SAFE_FREE(pathlist[i]->pathname);
			SAFE_FREE(pathlist[i]);
		}
	} else {
		for (i = 0; i < max_log_files; i++) {
			SAFE_FREE(pathlist[i]->pathname);
		...
		SAFE_FREE(pathlist[max_log_files]->pathname);
		SAFE_FREE(pathlist[max_log_files]);
	}
	SAFE_FREE(pathlist);
	SAFE_FREE(directory);
	sys_closedir(dir);
...
#ifdef ADD_LOG_SIZE_CONTROL
	if (check_log_files() < 0) {
		DEBUG(0, ("Error check log fiels when reopen logs.\n"));
	}
#endif
	new_dbf = x_fopen( debugf, O_WRONLY|O_APPEND|O_CREAT, 0640);
...
#ifdef ADD_LOG_SIZE_CONTROL
			if (check_log_files() < 0) {
				DEBUG(0, ("Error check log fiels when debug1.\n"));
			}
#endif
			dbf = x_fopen( debugf, O_WRONLY|O_APPEND|O_CREAT, 0640 );
			(void)umask( oldumask );
			if( dbf ) {
				x_setbuf( dbf, NULL );
			} else {
