...
static BOOL serverid_exists(struct server_id pid)
{
    DIR*    pDir;
    char acPath[128] = {0};
    snprintf(acPath, sizeof(acPath), "/%s/%d", "proc", pid.id.pid);
    pDir = opendir(acPath);
    if (!pDir)
    {
        return False;
...
NTSTATUS messaging_tdb_cleanup(struct server_id pid)
{
	TDB_DATA key;

	key = message_key_pid(pid.id);
	/*
	...
	if (!serverid_exists(pid)) {
		DEBUG(3,("pid %d doesn't exist - messaging_tdb_cleanup\n",(int)pid.id.pid));
		(void)tdb_delete(tdb, key);
	}
    else
    {
        DEBUG(3,("pid %d exist - why here\n",(int)pid.id.pid));
    }
...
*/
void messaging_cleanup_server(struct server_id server)
{
    /*
    ...
    */
	...
	(void)messaging_tdb_cleanup(server);
