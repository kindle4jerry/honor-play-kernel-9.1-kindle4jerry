...
extern int is_mac_os;
#define MAC_NAME_PREFIX "Mac OS X"
#ifdef NO_APPLEDOUBLE
int mac_os_flag = 0;
//for get mac_os_flag
int mac_os(void)
{
					...
					int length,int bufsize)
{
	...
	char* isMac = strstr_m(native_os, MAC_NAME_PREFIX);
	if(isMac != NULL){
		is_mac_os = 1;
#ifdef NO_APPLEDOUBLE
		char *p = strrchr_m(native_os,'.');
		if(p[1] != '\0') {
			fstring mac_os_version;
			fstrcpy(mac_os_version, p+1);
			int version_number = atoi(mac_os_version);
			if(version_number) {
				if(version_number > MAC_OS_DIFF_VERSION) {
					mac_os_flag |= MAC_OS_13;
				} else {
					mac_os_flag |= MAC_OS_12;
				}
			} else {
				mac_os_flag = 0;
			}
...
#else
	if ((blob1.data) && (blob1.data[0] == ASN1_APPLICATION(0))) {
#endif
...
#else
	if ((blob1.data) && (blob1.data[0] == ASN1_CONTEXT(1))) {
#endif
		/* its a auth packet */
...
#else
	if ((blob1.data) && (strncmp((char *)(blob1.data), "NTLMSSP", 7) == 0)) {
#endif
		DATA_BLOB chal;
