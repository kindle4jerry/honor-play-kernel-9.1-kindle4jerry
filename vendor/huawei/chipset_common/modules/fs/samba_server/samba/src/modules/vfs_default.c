...
/* BEGIN IDENTIFY FILE OPERATION */
#ifdef FOP_REFRESH
#define FILE_OP_TYPE_LEN 5

void send_fop_unix_socket(command_type cmd_type, int client_sockfd, int type,
	const char* dirpath, const char* path, const char* old_path)
{
	...
	data_tlv_t fop_type;
	data_tlv_t fop_name;
	data_tlv_t fop_old_name;
	data_tlv_t fop_dir_path;
	char str_type[FILE_OP_TYPE_LEN];

	switch (cmd_type) {
		case DEVICE_FILE_OPERATION:
			device_ip.type = DEVICE_IP;
			fop_type.type = FOP_TYPE;
			fop_dir_path.type = FOP_DIR_PATH;
			fop_name.type = FOP_PATH;
			fop_old_name.type = FOP_OLD_PATH;

			device_ip.value = client_addr();
			memset(str_type, '\0', FILE_OP_TYPE_LEN);
			sprintf(str_type, "%d", type);
			fop_type.value = str_type;
			fop_dir_path.value = dirpath;
			fop_name.value = path;
			fop_old_name.value = old_path;

			device_ip.length = strlen(device_ip.value);
			fop_type.length = strlen(fop_type.value);
			fop_dir_path.length = strlen(fop_dir_path.value);
			fop_name.length = strlen(fop_name.value);
			fop_old_name.length = strlen(fop_old_name.value);

			cmd_length = DATA_TYPE_SIZE + DATA_LEN_SIZE + sizeof(pid_t) +
						 DATA_TYPE_SIZE + DATA_LEN_SIZE + device_ip.length +
						 DATA_TYPE_SIZE + DATA_LEN_SIZE + fop_type.length +
						 DATA_TYPE_SIZE + DATA_LEN_SIZE + fop_dir_path.length +
						 DATA_TYPE_SIZE + DATA_LEN_SIZE + fop_name.length +
                         DATA_TYPE_SIZE + DATA_LEN_SIZE + fop_old_name.length;

			buffer_size = cmd_length + COMMAND_TYPE_SIZE + COMMAND_LEN_SIZE;
			buffer_send = (char *)SMB_MALLOC(buffer_size);

			DEBUG(3,("send_unix_socket: ip = %s, fop_type = %s, fop_dir_path = %s, fop_name = %s\n",
						device_ip.value, fop_type.value, fop_dir_path.value, fop_name.value));
			cur_pos = tlv_command_msg(buffer_send, cmd_type, cmd_length);
			cur_pos = add_data_tlv(buffer_send, cur_pos, (data_tlv_t*)&device_ip);
			cur_pos = add_data_tlv(buffer_send, cur_pos, (data_tlv_t*)&fop_type);
			cur_pos = add_data_tlv(buffer_send, cur_pos, (data_tlv_t*)&fop_dir_path);
			cur_pos = add_data_tlv(buffer_send, cur_pos, (data_tlv_t*)&fop_name);
			cur_pos = add_data_tlv(buffer_send, cur_pos, (data_tlv_t*)&fop_old_name);
			break;
		default:
			DEBUG(0,("send_unix_socket: Unknown command type! [%d]\n", cmd_type));
			return;
	}
	DEBUG(3,("send_unix_socket: command type! [%d]\n", cmd_type));
...
#define FNUM_MAP_SIZE 10
#define FNUM_MAP_DEFAULT_VALUE -1
typedef struct hashnode{
	  int value;
	  struct hashnode* nextValue;
	  struct hashnode* prevValue;
}HashNode;

HashNode fnumMap[FNUM_MAP_SIZE];

static int fnum_map_init(void)
{
	 int size = 0;
	 for(; size < FNUM_MAP_SIZE; size++)
	 {
		fnumMap[size].value = FNUM_MAP_DEFAULT_VALUE;
		fnumMap[size].nextValue = NULL;
		fnumMap[size].prevValue = NULL;
	 }
}

static HashNode* fnum_map_lookup(int value)
{
	 int key = value % FNUM_MAP_SIZE;
	 HashNode* node = &(fnumMap[key]);

	 for(; node->nextValue != NULL; node = node->nextValue)
	 {
	      if(node->value == value)
		     return node;
	 }
...
static int fnum_map_insert(int value)
{
	HashNode* node = fnum_map_lookup(value);

	//1.The value is already in map
	if(node->value == value)
		return -1;

	//2.Allocate a space for new value, then insert to the map
	HashNode* nnode = (HashNode*)malloc(sizeof(HashNode));
	nnode->value = value;
	nnode->nextValue = NULL;
	nnode->prevValue = node;
	node->nextValue = nnode;

	return 0;
}

static int fnum_map_remove(int value)
{
	HashNode* node = fnum_map_lookup(value);

	//1.The map dose not contain this value.
	if(node->value != value)
		return -1;

	//2. Remove this value from map, and free it's space
	HashNode* pnode = node->prevValue;
	HashNode* nnode = node->nextValue;
	pnode->nextValue = nnode;
	if(nnode != NULL)
		nnode->prevValue = pnode;
	free(node);

	return 0;
}

static int fnum_map_find(int value)
{
	 int key = value % FNUM_MAP_SIZE;
	 HashNode* node = fnumMap[key].nextValue;

	 for(; node != NULL; node = node->nextValue)
	 {
	      if(node->value == value)
		      return 1;
	 }
...
/* BEGIN IDENTIFY FILE OPERATION */
	   ...
	   if(result != -1)
	   {
			DEBUG(3,("vfswrap_mkdir %s, result = %d\n", path, result));
			send_fop_unix_socket(DEVICE_FILE_OPERATION, smbd_child_fd(), FOP_CREATE_DIR, handle->conn->connectpath, path, path);
	   }
#endif
/* END IDENTIFY FILE OPERATION */
	return result;
...
/* BEGIN IDENTIFY FILE OPERATION */
	   ...
	   if(result != -1)
	   {
		   DEBUG(3,("vfswrap_rmdir %s, result = %d\n", path, result));
		   send_fop_unix_socket(DEVICE_FILE_OPERATION, smbd_child_fd(), FOP_DELETE_DIR, handle->conn->connectpath, path, path);
	   }
#endif
/* END IDENTIFY FILE OPERATION */
...
/* BEGIN IDENTIFY FILE OPERATION */
#ifdef FOP_REFRESH
	if((result != -1)  && (flags & O_CREAT))
	{
		 DEBUG(3,("vfswrap_open %s, fd = %d, flags = %d, fnum = %d\n", fname, result, flags, fsp->fnum));
		 fnum_map_insert(fsp->fnum);
	}
#endif
/* END IDENTIFY FILE OPERATION */
...
/* BEGIN IDENTIFY FILE OPERATION */
#ifdef FOP_REFRESH
	   if((result != -1) && fnum_map_find(fsp->fnum))
	   {
		   DEBUG(3,("vfswrap_close %s, fd = %d, fnum = %d, result = %d\n", fsp->fsp_name, fd, fsp->fnum, result));
		   send_fop_unix_socket(DEVICE_FILE_OPERATION, smbd_child_fd(), FOP_CREATE_FILE, handle->conn->connectpath, fsp->fsp_name, fsp->fsp_name);
		   fnum_map_remove(fsp->fnum);
	   }
#endif
/* END IDENTIFY FILE OPERATION */
...
/* BEGIN IDENTIFY FILE OPERATION */
	...
	if(result != -1)
	{
		DEBUG(3,("vfswrap_rename, fd, oldname = %s, newname = %s, result = %d\n",oldname, newname, result));
		send_fop_unix_socket(DEVICE_FILE_OPERATION, smbd_child_fd(), FOP_RENAME, handle->conn->connectpath, newname, oldname);
	}
#endif
/* END IDENTIFY FILE OPERATION */
...
/* BEGIN IDENTIFY FILE OPERATION */
	...
	if(result != -1)
	{
		DEBUG(3,("vfswrap_unlink path = %s, dirpath =%s, connectpath = %s, origpath = %s\n",
			path, handle->conn->dirpath, handle->conn->connectpath, handle->conn->origpath));
		send_fop_unix_socket(DEVICE_FILE_OPERATION, smbd_child_fd(), FOP_DELETE_FILE, handle->conn->connectpath, path, path);
	}
#endif
/* END IDENTIFY FILE OPERATION */
