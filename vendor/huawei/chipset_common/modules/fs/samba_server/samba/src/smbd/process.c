...
static int active_msg_whitelist_map[256] = {0};
static int active_msg_whitelist_type[] = {
	0x00, /*SMBmkdir*/
	0x01, /*SMBrmdir*/
	0x02, /*SMBopen*/
	0x03, /*SMBcreate*/
...
#define ECHO_TIME_OUT 2*60
#define MAC_SELECT_TIMES 3
...
static BOOL timeout_processing(int deadtime, int *select_timeout, time_t *last_timeout_processing_time)
{
...
#ifdef ADD_BUGFIX2
	static int smb_select_timeout_count = 0;
#endif
	time_t t;
	BOOL allidle = True;
	...
	*last_timeout_processing_time = t = time(NULL);
	...
	DEBUG( 12, ( "\ttime processing, conn_num_open=%d\n", conn_num_open()));
	if (keepalive && (t - last_keepalive_sent_time)>keepalive) {
		DEBUGADD( 12, ( "\tKeepalive check.\n" ) );
		if (!send_keepalive(smbd_server_fd())) {
			DEBUGADD( 12, ( "\tKeepalive failed - exiting.\n" ));
			return False;
		}
		DEBUGADD( 12, ( "\tsend_keepalive(smbd_server_fd())  success \n" ) );
	...
	if (allidle && conn_num_open()>0) {
		...
		if((is_mac_os == 1) && (mac_idle_flag == 0)) {
			send_unix_socket(COMPUTER_DEVICE_INACTIVE, smbd_child_fd(), NULL);
			mac_idle_flag = 1;
			DEBUG(2,("Mac os idle send inactive for allidle\n"));
		}
		else if(is_mac_os != 1){
			return False;
		}
	...
	if(global_machine_password_needs_changing && 
		...
		if (secrets_lock_trust_account_password(lp_workgroup(), True) == False) {
			DEBUGADD(12,("process: unable to lock the machine account password for \
machine %s in domain %s.\n", global_myname(), lp_workgroup() ));
			return True;
		}

		if(!secrets_fetch_trust_account_password(lp_workgroup(), trust_passwd_hash, &lct, NULL)) {
			DEBUGADD(12,("process: unable to read the machine account password for \
machine %s in domain %s.\n", global_myname(), lp_workgroup()));
	...
	DEBUGADD( 12, ( "\tprocess_blocking_lock_queue finish \n" ) );
	...
	check_log_size();
	DEBUGADD( 12, ( "\tforce_check_log_siz finish \n" ) );
...
void smbd_process(void)
{
	...
	int echo_timeout = 0;
	int echo_timeout_flag = 0;
	int mac_selec_timeout = 0;
	time_t windows_echo_processing_time = time(NULL);

	int active_msg_whitelist_index = 0;
	int active_msg_whitelist_pos = 0;
	int active_msg_whitelist_size = sizeof(active_msg_whitelist_type)/sizeof(int);
	for( active_msg_whitelist_index = 0; active_msg_whitelist_index < active_msg_whitelist_size; active_msg_whitelist_index++) {
		active_msg_whitelist_pos = active_msg_whitelist_type[active_msg_whitelist_index];
		active_msg_whitelist_map[active_msg_whitelist_pos] = 1;
	}
	...
	while (True) {
		...
		DEBUG(10, ("\t enter smbd_process while\n"));
		/* Did someone ask for immediate checks on things like blocking locks ? */
		if (select_timeout == 0) {
			DEBUGADD(10,("\tDid someone ask for immediate checks timeout_processing!\n"));
		...
		while (!receive_message_or_smb(InBuffer,BUFFER_SIZE+LARGE_WRITEX_HDR_SIZE,select_timeout)) {
			DEBUGADD(10,("\twhile (!receive_message_or_smb(InBuff...    timeout_processing!\n"));
			if (++mac_selec_timeout == MAC_SELECT_TIMES) {
				DEBUGADD(10,("timeout_processing: receive_smb sys_select timeout\n"));
				if((is_mac_os == 1) && (mac_idle_flag == 0)) {
					send_unix_socket(COMPUTER_DEVICE_INACTIVE, smbd_child_fd(), NULL);
					mac_idle_flag = 1;
					DEBUGADD(10,("Mac os idle send inactive for smb_read_error\n"));
				}
				mac_selec_timeout = 0;
			}
			if(!timeout_processing( deadtime, &select_timeout, &last_timeout_processing_time))
		...
		//SMBecho : for windows
		msg_type = CVAL(InBuffer,smb_com);

		//Check receive five continuous SMBecho
		if((msg_type == SMBecho) && (echo_timeout_flag == 0)) {
			time_t echo_check_time = time(NULL);
			if(++echo_timeout==1) {
				windows_echo_processing_time = time(NULL);
			}
			if((echo_check_time-windows_echo_processing_time) >= ECHO_TIME_OUT) {
				echo_timeout = 0;
				DEBUGADD(10, (("\tclient receive five continuous SMBecho inactive\n")));
#ifdef ADD_UNIX_SOCKET
				send_unix_socket(COMPUTER_DEVICE_ECHO_INACTIVE, smbd_child_fd(), NULL);
#endif
				echo_timeout_flag = 1;

			}
		} else {
			//check SMBtrans2 subcommand GET_DFS_REFERRAL: for windows (16 min peroid)
			if((active_msg_whitelist_map[msg_type] == 1) && ((echo_timeout_flag == 1) || (mac_idle_flag == 1))) {//flag 1:inactive
					DEBUGADD(10, (("\tclient resume active\n")));
#ifdef ADD_UNIX_SOCKET
					send_unix_socket(COMPUTER_DEVICE_ACTIVE, smbd_child_fd(), NULL);
#endif
					echo_timeout_flag = 0;
					mac_idle_flag = 0;
			}
		...
		if (smb_echo_count != num_echos) {
			DEBUGADD(10,("\tsmb_echo_count != num_echos    timeout_processing!\n"));
			if(!timeout_processing( deadtime, &select_timeout, &last_timeout_processing_time))
				return;
			num_smbs = 0; /* Reset smb counter. */
		}
		...
		if ((num_smbs % 200) == 0) {
			...
			if(new_check_time - last_timeout_processing_time >= (select_timeout/1000)) {
				DEBUGADD(10,("\tnum_smbs % 200) == 0   timeout_processing!\n"));
				if(!timeout_processing( deadtime, &select_timeout, &last_timeout_processing_time))
					return;
				num_smbs = 0; /* Reset smb counter. */
