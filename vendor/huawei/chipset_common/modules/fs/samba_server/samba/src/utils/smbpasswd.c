...
{
#ifdef CUT_SMBPASSWD
	printf("Run only in format : smbpasswd -n/-o username password\n");
#else
...
#ifdef CUT_SMBPASSWD
	while ((ch = getopt(argc, argv, "noh")) != EOF) {
		switch(ch) {
		case 'n':
...
#else
	ret = local_password_add(username, local_flags, new_pw, 
				     err_str, sizeof(err_str), msg_str, sizeof(msg_str));
#endif
