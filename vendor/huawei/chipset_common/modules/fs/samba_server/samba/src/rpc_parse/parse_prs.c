...
{	
    ...
    DEBUG(5,("start %d, len %u, src %u %u %u -> dst %u %u %u\n", start, len, src->data_offset, src->buffer_size, src->grow_size,
        dst->data_offset, dst->buffer_size, dst->grow_size));

    //src->buffer_size < start : 确保后面的计算不会反转
    //src->buffer_size - start < len Samba 3.0.37 EnumPrinters 堆内存溢出漏洞
    if (src->buffer_size < start || src->buffer_size - start < len) {
        DEBUG(0,("Buffer overflow - unable to memcpy buffer by %u bytes.\n", len));
        return False;
    }
#endif
