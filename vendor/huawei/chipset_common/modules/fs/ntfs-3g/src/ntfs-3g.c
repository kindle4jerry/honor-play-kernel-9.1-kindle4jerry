...
static struct fuse *mount_fuse(char *parsed_options)
{
	...
	if (fuse_opt_add_arg(&args, "") == -1)
		goto err;
#if !CACHEING
	if (fuse_opt_add_arg(&args, "-ouse_ino,kernel_cache,attr_timeout=0,hard_remove") == -1)
		goto err;
#else
	if (fuse_opt_add_arg(&args, "-ouse_ino,kernel_cache,attr_timeout=1,hard_remove") == -1)
		goto err;
#endif
	if (ctx->debug)
