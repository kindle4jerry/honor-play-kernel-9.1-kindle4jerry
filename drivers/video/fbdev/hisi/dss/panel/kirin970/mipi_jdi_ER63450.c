...
static struct dsi_cmd_desc lcd_display_on_video_cmds[] = {
...
static struct dsi_cmd_desc lcd_display_on_cmd_cmds[] = {
	{DTYPE_DCS_WRITE1, 0, 10, WAIT_TYPE_US,
		sizeof(bl_enable), bl_enable},
	{DTYPE_DCS_WRITE1, 0, 10, WAIT_TYPE_US,
		sizeof(te_enable), te_enable},
...
static int mipi_jdi_panel_on(struct platform_device *pdev)
{
	...
	if (pinfo->lcd_init_step == LCD_INIT_POWER_ON) {
		...
		if (pinfo->type == PANEL_MIPI_CMD) {
			mipi_dsi_cmds_tx(lcd_display_on_cmd_cmds, \
				ARRAY_SIZE(lcd_display_on_cmd_cmds), mipi_dsi0_base);
		} else {
			mipi_dsi_cmds_tx(lcd_display_on_video_cmds, \
				ARRAY_SIZE(lcd_display_on_video_cmds), mipi_dsi0_base);
		}
	...
	char *buf)
{
	...
	ret = snprintf(buf, PAGE_SIZE, "JDI_ER63450 ' CMD TFT Boston\n");
...
static int mipi_jdi_probe(struct platform_device *pdev)
{
	...
	pinfo->type = PANEL_MIPI_CMD;//PANEL_MIPI_CMD;
	...
	if (g_lcd_fpga_flag == 1) {
		...
		pinfo->mipi.dsi_bit_clk_val1 = 120;
