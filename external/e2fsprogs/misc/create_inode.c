...
const char* *no_sparse_file_type_list=NULL;
static int file_type_list_count=0;
#define USE_SPARSE_SAVE_FILE 0
#define NO_SPARSE_SAVE_FILE 1

/*******************************************************
*FunctionNmae:  check_sparse_filetype
*Description :  check file whether is  ext filename
*Parameters: filename file ready to input to new system
*return vale: none
********************************************************/
static void check_sparse_filetype(const char* filename)
{
	...
	if (NULL == filename || 0 == file_type_list_count || NULL == no_sparse_file_type_list)
	{
		return;
	}
	...
	for(idex=0; idex < file_type_list_count; idex++)
	{
		...
		if (0 == strncmp(extfilename, no_sparse_file_type_list[idex], extfnlen))
		{
...
/*******************************************************
*FunctionNmae:  check_file_use_sparse
*Description :  check file save data type
*Parameters: none
*return vale: USE_SPARSE_SAVE_FILE  can use sparse style
*             NO_SPARSE_SAVE_FILE   not use sparse style
********************************************************/
static int check_file_use_sparse()
{
	return is_no_sparse_file;
}

/*******************************************************
 *FunctionNmae:  set_no_sparse_file_list
 *Description :  set special file type from the caller
 *Parameters  :  nexttype:  file type list
 *               typecount: count of file type
 *return vale:   none
 ********************************************************/
void set_no_sparse_file_list(const char** exttype, int typecount)
{
	if (0 == typecount || NULL == exttype)
	{
		return;
	}
	...
	for (off = start; off < end; off += COPY_FILE_BUFLEN) {
		...
		for (bpos = 0, ptr = buf; bpos < got; bpos += fs->blocksize) {
			...
			if (blen > got - bpos)
				blen = got - bpos;
#ifdef HW_FILE_SAVE_NO_SPARSE
			if (USE_SPARSE_SAVE_FILE == check_file_use_sparse())
			{
#endif
