 ...
 * bw_penalty_box or bw_happy_box, no need to insert and append.
 */
static const char *unique_chian[] = {
	"mobile_reject_list",
	"wifi_reject_list",
	"bw_penalty_box",
	"bw_happy_box",
...
bool is_chain_unique(const char *chain)
{
	if (chain == NULL) {
		return false;
	}

	for (int i = 0; unique_chian[i]; ++i) {
		if (!strncmp(unique_chian[i], chain, strlen(unique_chian[i]))) {
			return true;
		}
	}
	return false;
