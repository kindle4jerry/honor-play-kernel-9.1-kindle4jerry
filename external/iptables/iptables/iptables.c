		...
		struct xtc_handle **handle, bool restore)
{
	...
	case CMD_APPEND:
		if (is_chain_unique(chain)) {
			...
			if (ret) {
				printf("%s: keep the rules unique in chain: %s[iptables APPEND]", __FUNCTION__, chain);
			}
		} else {
			ret = 0;
		}
	...
	case CMD_INSERT:
		if (is_chain_unique(chain)) {
			ret = check_entry(chain, e,
				...
				*handle, cs.matches, cs.target);
			if (ret) {
				printf("%s: keep the rules unique in chain: %s[iptables INSERT]", __FUNCTION__, chain);
			}
		} else {
			ret = 0;
		}
