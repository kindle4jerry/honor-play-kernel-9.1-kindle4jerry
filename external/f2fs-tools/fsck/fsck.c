	...
	for (idx = 0; idx < 5; idx++) {
		...
		if (_nid != 0 && IS_VALID_NID(sbi, _nid)) {
			...
			if (IS_VALID_BLK_ADDR(sbi, _ni.blk_addr))
				queue_reada_block(sbi, _ni.blk_addr, NODE);
		}
	}
		...
		struct child_info *child)
{
	...
	/* readahead node blocks, skip the 1st one */
	for (i = 1; i < NIDS_PER_BLOCK; i++) {
		u32 _nid = le32_to_cpu(node_blk->in.nid[i]);
...
int fsck_chk_meta(struct f2fs_sb_info *sbi)
{
	...
	/* 6. check nat entry with sit_area_bitmap */
