...
int sanity_check_raw_super(struct f2fs_super_block *sb, u64 offset)
{
	...
	else if (f2fs_crc_valid(crc, sb, CHECKSUM_OFFSET_SB)) {
		MSG(0, "Invalid CRC checksum: 0x%x\n", crc);
		return -1;
	}
...
int validate_super_block(struct f2fs_sb_info *sbi, int block)
{
	...
	if (!sanity_check_raw_super(sbi->raw_super, offset)) {
		...
		if (cmp_kernel_version(c.sb_version, c.version)) {
			...
			if (old_crc) {
				crc = f2fs_cal_crc32(F2FS_SUPER_MAGIC,
							sbi->raw_super,
							CHECKSUM_OFFSET_SB);
				sbi->raw_super->crc = cpu_to_le32(crc);
				MSG(0, "Info: update CRC successfully"
					"(0x%x --> 0x%x)\n", old_crc, crc);
			}
				...
				unsigned long long *version)
{
	...
	if (get_cp(cp_pack_total_block_count) > sbi->blocks_per_seg) /*lint !e737*/
		goto invalid_cp1;
...
static void read_compacted_summaries(struct f2fs_sb_info *sbi)
{
	...
	memcpy(&curseg->journal->n_nats, kaddr, SUM_JOURNAL_SIZE); //lint !e419
...
int f2fs_do_mount(struct f2fs_sb_info *sbi)
{
	...
	if (c.auto_fix || c.preen_mode) {
		...
		if (flag & CP_FSCK_FLAG || flag & CP_QUOTA_NEED_FSCK_FLAG ||
